...
StdoutToVar_Blob(sCmd, ByRef sOut, sEncoding:=0, sDir:="", ByRef nExitCode:=0) {
	DllCall( "CreatePipe",           PtrP,hStdOutRd, PtrP,hStdOutWr, Ptr,0, UInt,0 )
	DllCall( "SetHandleInformation", Ptr,hStdOutWr, UInt,1, UInt,1                 )
	...
	While(DllCall("ReadFile", Ptr,hStdOutRd, Ptr,&buffer, UInt, nBuf, PtrP,nSize, Ptr,0 ))
	{
		DllCall("RtlMoveMemory", Ptr, sPtr, Ptr, &buffer, UInt, nSize)
		sPtr += nSize
	}
	totalBytes := sPtr - &sOut
    ...
    DllCall( "CloseHandle",        Ptr,hStdOutRd                     )
	if(sEncoding) 
		sOut := StrGet(&sOut, totlaBytes, sEncoding)
	Return totalBytes
}
